...
suite('locDefinition', () => {
  test('resolves the loc key under the cursor inside a quoted string', () => {
    ...
    const resolved = resolveLocKeyAt(analyze(text), offset, index);
  ...
  test('reports the token range for hover highlighting', () => {
    const text = 'name = o';
    const resolved = resolveLocKeyAt(analyze(text), text.length - 1, index);
    assert.ok(resolved, 'expected a resolved key');
  ...
  test('returns undefined for unknown words and non-word positions', () => {
    ...
    assert.strictEqual(resolveLocKeyAt(analyze(text), text.indexOf('UNKNOWN') + 2, index), undefined);
    assert.strictEqual(resolveLocKeyAt(analyze(text), text.length - 1, index), undefined);
  });

  test('hover markdown shows the text and the source line', () => {
    const text = 'title = "EVTNAME100"';
    const resolved = resolveLocKeyAt(analyze(text), 12, index);
    assert.ok(resolved, 'expected a resolved key');
  ...
  test('hover markdown marks empty translations', () => {
    ...
    const resolved = resolveLocKeyAt(analyze('title = EMPTY_KEY'), 12, emptyIndex);
    assert.ok(resolved, 'expected a resolved key');
